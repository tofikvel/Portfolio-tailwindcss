    ...
    <nav className="bg-sky-50 shadow-sm">
      <div className="max-w-6xl mx-auto px-6 md:px-0">
        <div className="flex justify-between items-center h-20">
          ...
          >
            <span
              className={`absolute h-0.5 w-6 bg-black dark:bg-blue-500 transition-transform duration-300 ${menuOpen ? "rotate-45" : "-translate-y-2"
                }`}
            />
            <span
              className={`absolute h-0.5 w-6 bg-black dark:bg-blue-500 transition-opacity duration-300 ${menuOpen ? "opacity-0" : ""}`}
            />
            <span
              className={`absolute h-0.5 w-6 bg-black dark:bg-blue-500 transition-transform duration-300 ${menuOpen ? "-rotate-45" : "translate-y-2"
                }`}
